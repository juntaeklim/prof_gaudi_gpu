...
def run():
    ...
    print("Result of (input_size, dim_size) = (%d, %d) for %s" %(input_size, dim_size, method))
    print()
    if method == "vtrain":
    ...
    for index_size in index_sizes:
        file_name = "%s/%s_input_%d_index_%d_dim_%d.txt" %(log_path, method, input_size, index_size, dim_size)
