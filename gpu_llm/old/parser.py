...
def main():
    ...
    while batch_size <= 128:
        if dtype == "fp32":
            log_filename = f"./logs/{model_name}_batch_{batch_size}_in_{input_length}_out_{output_length}_trt.txt"
        else:
            log_filename = f"./logs/{model_name}_batch_{batch_size}_in_{input_length}_out_{output_length}_{dtype}_trt.txt"
