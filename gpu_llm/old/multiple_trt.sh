...
for length in "${lengths[@]}"; do
  ...
  for model in "${models[@]}"; do
    ...
    for batch in "${batch_sizes[@]}"; do
      ...
      if [ "$dtype" == "fp32" ]; then
        log_file="./logs/${model_name}_batch_${batch}_in_${I}_out_${O}_trt.txt"
      else
        log_file="./logs/${model_name}_batch_${batch}_in_${I}_out_${O}_${dtype}_trt.txt"
      fi
      ...
      echo "Running: python run_opt_trt.py --model $model --batch-size $batch --dtype $dtype --input-length $I --output-length $O"
      python run_opt_trt.py --model $model --batch-size $batch --dtype $dtype --input-length $I --output-length $O > "$log_file" 2>&1
